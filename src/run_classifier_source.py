...
class MnliProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "11211/train.tsv")), "train"
        )

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "11211/dev.tsv")), "dev"
        )

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "11211/test.tsv")), "test"
        )
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer):
    ...
    tokens_a = tokenizer.tokenize(example.text_a.strip())
    tokens_b = None
    if example.text_b:
        tokens_b = tokenizer.tokenize(example.text_b.strip())
...
def _truncate_seq_pair(tokens_a, tokens_b, max_length, max_length_a=56, max_length_b=10):
    ...
    while True:
        ...
        if total_length <= max_length:
            break
        if max_length < total_length <= max_length + 3:
            tokens_a = tokens_a[:-2]
        ...
        if len(tokens_b) > max_length_b:
            tokens_b.pop()
        elif len(tokens_a) > max_length_a:
            tokens_a.pop()
...
def create_model(
    ...
    # instead.
    output_layer = model.get_pooled_output()  # [CLS] output
    ...
    with tf.variable_scope("loss"):
        ...
        output_layer = tf.layers.dense(output_layer, units=hidden_size)  # Full Connection Layer
